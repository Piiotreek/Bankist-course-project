...
// for (const movement of movements) {
  ...
  if (movement > 0) {
    console.log(`Movement ${i+1}: You depozited ${movement}`);
  } else {
...
movements.forEach(function (mov,i,arr) {
  if (mov > 0) {
    console.log(`Movement ${i+1}: You depozited ${mov}`);
  } else {
...
const checkDogs = function(dogsJulia,dogsKate){
  ...
  ;
  // console.log(dogsJulia.slice(1,-1));
  // console.log(dogsKate.slice(1,-1));
  const correctedArr = dogsJulia.slice(1,-2).concat(dogsKate.slice());
  console.log(correctedArr);
  correctedArr.forEach(function(value,key){
    if(value<3){
      console.log(`Dog number  is still a puppy 🐶`);
    }else{
      console.log(`Dog number ${key+1} is an adult, and is ${value} years old`);
    }
...
//acccumulator is like a snowball
...
const calcAverageHumanAge = function (ages) {
  ...
  console.log(`All the dogs in human age: ${humanAge}`);
  const excludeAdults = humanAge.filter(function (age) {
    return age >= 18;
  });
  console.log(`These are dogs that are adult: ${excludeAdults}`);
  const adultAvg = excludeAdults.reduce(function (acc, age) {
    return acc + age / excludeAdults.length;
  }, 0);
  console.log(`That is a avg age of adult dog: ${adultAvg}`);
};
...
const avg = calcAverageHumanAge([16, 6, 10, 5, 6, 1, 4]);
console.log(avg);
...
// dogs.forEach(
//   portion => (portion.recomendedFood = Math.trunc(portion.weight ** 0.75 * 28))
// );
// console.log(dogs);
...
//     sarahDog.curFood > sarahDog.recomendedFood * 0.9 &&
//     sarahDog.curFood < sarahDog.recomendedFood * 1.1
//       ? `okey`
//       : `too much`
//   } `
// );
...
//   .filter(food => food.curFood > food.recomendedFood)
//   .map(dog => dog.owners)
//   .flat();
...
// const copyDogs = dogs.slice().sort((a,b)=>a.recomendedFood-b.recomendedFood)
